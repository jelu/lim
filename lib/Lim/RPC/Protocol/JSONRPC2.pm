...
sub handle {
    ...
    if ($request->header('Content-Type') =~ /(?:^|\s)application\/json(?:$|\s|;)/o and $request->uri =~ /^\/([a-zA-Z]+)\s*$/o) {
        ...
        if (defined $server and $server->have_module($module)) {
            ...
            eval { $jsonreq = $JSON->decode($request->content); };
            unless ($@) {
                            ...
                            Lim::RPC::Callback->new(
                                ...
                                cb      => sub {
                                    ...
                                    if (blessed $result and $result->isa('Lim::Error')) {
                                        ...
                                        eval { $response->content($JSON->encode({jsonrpc => '2.0', error => {code => $result->code, message => $result->message}, id => $id})); };
                                        if ($@) {
                                        ...
                                        eval { $response->content($JSON->encode({jsonrpc => '2.0', result => $result, id => $id})); };
                                        if ($@) {
                                            $response->code(HTTP_INTERNAL_SERVER_ERROR);
                                            Lim::WARN and $self->{logger}->warn('JSON encode error: ', $@);
                                        }
            ...
            if (defined $jsonresp) {
                eval { $response->content($JSON->encode($jsonresp)); };
                if ($@) {
                    $response->code(HTTP_INTERNAL_SERVER_ERROR);
                    Lim::WARN and $self->{logger}->warn('JSON encode error: ', $@);
                }
...
sub request {
    ...
    if (defined $args{data}) {
        eval { $request->content($JSON->encode({jsonrpc => '2.0', id => $ID++, method => $args{call}, params => $args{data}})); };
        if ($@) {
