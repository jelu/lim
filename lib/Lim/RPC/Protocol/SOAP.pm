...
sub handle {
    ...
    if ($request->header('SOAPAction') and $request->uri =~ /^\/([a-zA-Z]+)\s*$/o) {
                    ...
                    $request->{__lim_rpc_protocol_soap_cb} = Lim::RPC::Callback->new(
                        ...
                        cb      => sub {
                            ...
                            if (blessed $data and $data->isa('Lim::Error')) {
                                ...
                                if (defined $data) {
                                    $result = $soap->serializer->prefix('s')->uri($method_uri)->envelope(response => $method_name . 'Response', SOAP::Data->value(__soap_result('base', $data)));
                                }
                                else {
                                    $result = $soap->serializer->prefix('s')->uri($method_uri)->envelope(response => $method_name . 'Response');
        ...
        if (defined $server and $server->have_module($module) and exists $self->{wsdl}->{$server->module_class($module)}) {
            ...
            $response->content($wsdl->[0] . $uri->as_string . $wsdl->[1]);
            $response->header(
        ...
        if (ref($_[1]->{$k}) eq 'ARRAY') {
            foreach my $v (@{$_[1]->{$k}}) {
                if (ref($v) eq 'HASH') {
                    push(
                        @a,
                        SOAP::Data->new->name($k)->value(Lim::RPC::__soap_result($_[0] . '.' . $k, $v))
                    );
                }
                else {
                    push(
                        @a,
                        SOAP::Data->new->name($k)->value($v)
                    );
                }
            }
        }
            ...
            push(
                @a,
                SOAP::Data->new->name($k)->value(Lim::RPC::__soap_result($_[0] . '.' . $k, $_[1]->{$k}))
            );
        }
        else {
            push(
                @a,
                SOAP::Data->new->name($k)->value($_[1]->{$k})
            );
        }
    }
