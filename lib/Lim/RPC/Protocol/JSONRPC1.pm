                            ...
                            Lim::RPC::Callback->new(
                                ...
                                cb      => sub {
                                    ...
                                    if (blessed $result and $result->isa('Lim::Error')) {
                                        ...
                                        eval { $response->content($JSON->encode({result => undef, error => {code => $result->code, message => $result->message}, id => $id})); };
                                        if ($@) {
                                        ...
                                        eval { $response->content($JSON->encode({result => $result, error => undef, id => $id})); };
                                        if ($@) {
                                            $response->code(HTTP_INTERNAL_SERVER_ERROR);
                                            Lim::WARN and $self->{logger}->warn('JSON encode error: ', $@);
                                        }
...
sub request {
    ...
    if (defined $args{data}) {
        eval { $request->content($JSON->encode({id => $ID++, method => $args{call}, params => $args{data}})); };
        if ($@) {
