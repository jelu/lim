                    ...
                    $request->{__lim_rpc_protocol_xmlrpc_cb} = Lim::RPC::Callback->new(
                        ...
                        cb      => sub {
                            ...
                            if (blessed $data and $data->isa('Lim::Error')) {
                                ...
                                if (defined $data) {
                                    $result = $xmlrpc->serializer->prefix('s')->uri($method_uri)->envelope(response => $method_name . 'Response', __xmlrpc_result('base', $data));
                                }
                                else {
                                    $result = $xmlrpc->serializer->prefix('s')->uri($method_uri)->envelope(response => $method_name . 'Response');
                                }
